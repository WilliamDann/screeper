...
function findWorkRole(creep: Creep, site: Site, f: Function)
{
    // Fill containers
    // TODO 
    ...
    containers = filter(containers, x => f(x) as boolean)
    ...
    sites     = filter(sites, x => f(x) as boolean);
    if (sites.length != 0)
    {
    ...
    controllers = filter(controllers, x => f(x) as boolean);
...
export default function(f: Function)
{
    return (function (creep:Creep) {
        ...
        if (!role || role == 'idle' || !state)
        {
            ...
            return findWorkRole(creep, this, f);
        }
    })as RoleHandler;
}